...
static void MyInterruptionCallback(void* user_data, UInt32 interruption_state)
{
	...
	printf("\nMyInterruptionCallback\n");
	OpenALSoundController* openal_sound_controller = (__bridge OpenALSoundController*)user_data;
...
- (EWSoundBufferData*) soundBufferDataFromFileBaseName:(NSString*)sound_file_basename
{
	...
	return [self soundBufferDataFromFileBaseName: sound_file_basename withCaching: YES];
}

- (EWSoundBufferData*) soundBufferDataFromFileBaseName:(NSString*)sound_file_basename withCaching: (BOOL)bCache;

{
	...
	void* pcm_data_buffer = MyGetOpenALAudioDataAll((__bridge CFURLRef)file_url, &data_size, &al_format, &sample_rate);
	[file_url release];
